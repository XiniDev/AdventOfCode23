...
std::string d7p2(const std::vector<std::string>& p_lines)
{
    int winnings = 0;
    ...
    for (const std::string& line : p_lines)
    {
        ...
        if (pos != std::string::npos)
        {
            ...
            char prev;
            int rep = 0;
            std::vector<std::pair<int, char>> repVec;
            for (const auto& c: sortedCards)
            {
                if (c == prev || rep == 0) rep++;
                else
                {
                    repVec.push_back(std::make_pair(rep, prev));
                    rep = 1;
                }
                prev = c;
            }
            ...
            auto it = std::find_if(std::begin(repVec), std::end(repVec), [] (const auto& pair)
            {
                return pair.second == 'J';
            });
            if (it != std::end(repVec))
            {
                int jokerReps = (*it).first;
                if (jokerReps != 5)
                {
                    std::erase_if(repVec, [] (const auto& pair)
                    {
                        return pair.second == 'J';
                    });
                    auto highestRep = std::max_element(std::begin(repVec), std::end(repVec), [] (const auto& lhs, const auto& rhs)
                    {
                        return lhs.first < rhs.first;
                    });
                    if (highestRep != std::end(repVec))
                        highestRep->first += jokerReps;
                }
            }
            ...
            switch (repVec.size())
            {
                case 1:
                    ...
                    break;
                case 2:
                    if (std::find_if(std::begin(repVec), std::end(repVec), [] (const auto& pair)
                    {
                        return pair.first == 4;
                    }) != std::end(repVec))

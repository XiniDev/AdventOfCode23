...
std::string d3p2(const std::vector<std::string>& p_lines)
{
    int sum = 0;
    int lineNum = 0;
    ...
    std::unordered_map<std::pair<int, int>, std::pair<int, int>, PairHash, PairEqual> adjMap;

    for (const auto& pair: numMap)
    {
        bool found = false;
        int numSize = static_cast<int>(log10(pair.second) + 1);
        std::pair<int, int> symPos;
        ...
        if (found)
        {
            auto it = adjMap.find(symPos);
            if (it != adjMap.end())
            {
                if (it->second.second < 2)
                    it->second.first = it->second.first * pair.second;
                else
                    it->second.first = 0;
                it->second.second += 1;
            }
            else
                adjMap[symPos] = std::make_pair(pair.second, 1);
        }
    }

    for (const auto& pair: adjMap)
    {
        if (pair.second.second == 2) sum += pair.second.first;
    }
